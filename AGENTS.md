...
- `extensions/subagent/notices.ts` delivers a parallel background run's `<subagent_result>` (built by `tool.ts` `resultBlock()`, `triggerTurn: true`) as soon as it finishes while others still run, and marks chain steps with one-line `<subagent_notification>` custom messages (`{"run","status"}`, `triggerTurn: false`). The change that ends a job is reported by the job's final message (`finalAnswers()`, which skips runs marked `delivered`). `extensions/codex-server-compaction/remote-compaction.ts` keys on the notification tag to keep notices out of V2 retained user messages; change the tag in both.
