...
- `extensions/` — user-level TypeScript extensions loaded by Pi.
  ...
  - `subagent/` — this repository's subagent implementation: single/parallel/chain delegation to `pi` subprocesses, sync or `async: true` background jobs, persisted child sessions under `<agent-dir>/subagent-sessions/`, a below-editor run panel with keyboard selection, live transcript and `/subagent-history` overlays, strict project-agent confirmation, and the `/subagent` TUI that updates user-agent model/thinking frontmatter. `agents.ts` and part of `runner.ts` are derived from Pi's example.
