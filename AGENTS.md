...
- `extensions/subagent/status.ts` appends a request-local `<system_status>` user message through the `context` event while subagents are active; it is never persisted. `extensions/codex-server-compaction` keys on the same `<system_status>` tag (`REQUEST_LOCAL_STATUS_TAG`): it refuses `previous_response_id` continuation after a request that carried it and excludes it from V2 retained messages. Change the tag in both; `subagent/status.test.ts` checks they match.
