...
- `subagent/` — delegates tasks to `pi` subprocesses in single, parallel, or chain mode, in the foreground or with `async: true` in the background; saves every child session, lists running subagents below the editor (`↓` on an empty prompt selects one to watch or stop), returns each run's short ID and session file so the main agent can `read` its progress and message, interrupt, continue, or stop a run with `subagent_control`, appends a one-line `<subagent_notification>` when a background run changes state, delivers a job's final answers together, and adds `/subagent-history` plus the `/subagent` model/thinking selector ([details](extensions/subagent/README.md))
