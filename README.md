...
- `subagent/` — delegates tasks to `pi` subprocesses in single, parallel, or chain mode, in the foreground or with `async: true` in the background; saves every child session, lists running subagents below the editor (`↓` on an empty prompt selects one to watch or stop), lets the main agent list, inspect, read, and wait for its runs with `subagent_control` and appends a one-line `<subagent_notification>` when a run of a multi-run background job finishes, and adds `/subagent-history` plus the `/subagent` model/thinking selector ([details](extensions/subagent/README.md))
