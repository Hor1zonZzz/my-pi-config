...
- **Add account / sign in again**：使用当前 provider 的公开 OAuth 实现（本配置中为
  ...
  授权 URL，或选择设备码流程；取消不保存。新增账号不会自动切换；重新授权当前账号
  则更新该账号的有效凭据。
...
- 已经绑定认证或发出的请求继续使用旧账号；其他 Pi 0.85.1 进程在下一次读取认证时跟随新账号。
...
针对 Pi **0.85.1** 验证。命令、对话框、OAuth 登录/刷新、模型目录刷新和事件使用公开 API。
...
Pi 0.85.1 会检查认证文件 revision，因此无需伪造 `expires: 0`，也无需每次切换都 reload。
