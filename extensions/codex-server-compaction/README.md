...
The [subagent extension](../subagent/README.md#live-status-in-every-request) may end a request with a request-local `<system_status>` user item that the next request drops. Because a response continued with `previous_response_id` keeps its request's input, the transport never continues a response whose request carried that item (not even through the pending tool-output shortcut) and sends full input instead; V2 retention never keeps it.
