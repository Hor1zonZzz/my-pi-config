...
function estimateCompactContextTokens(request: NativeCompactionRequestBody): number {
	return estimateTokenCount(request.instructions ?? "") + estimateTokenCount(request.input);
}
...
export async function shrinkNativeCompactionRequestForEndpoint(
	...
	const estimatedTokensBefore = estimateCompactContextTokens(request);
	...
	for (let index = request.input.length - 1; index >= 0 && estimatedTokensAfter > budgetTokens; index--) {
		...
		rewrittenOutputs++;
		estimatedTokensAfter += estimateTokenCount(rewrite.item) - estimateTokenCount(item);
	}
