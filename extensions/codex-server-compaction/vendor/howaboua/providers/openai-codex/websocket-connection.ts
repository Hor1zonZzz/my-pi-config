...
async function getWebSocketConstructor(_url: string, _env?: ProviderEnv): Promise<WebSocketConstructorLike | null> {
...
export async function resolveWebSocketProxyForTarget(url: string, env?: ProviderEnv): Promise<string | undefined> {
	...
	return withScopedProxyEnv(env, () => {
		const protocol = new URL(target).protocol;
		return (protocol === "https:" ? process.env.HTTPS_PROXY ?? process.env.https_proxy : process.env.HTTP_PROXY ?? process.env.http_proxy)
