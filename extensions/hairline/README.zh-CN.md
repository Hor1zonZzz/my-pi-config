...
─ gpt-5.6-sol · medium ───────────────────────── context ▰▰▰▰▰▰▰▰▰▰ 42% ─
  ~/workspace/my-pi-config  main       ↑12k ↓3.1k R28k CH99.5%  ·  $0.000 sub  ·  <其他状态>
```
...
| Footer | `ctx.ui.setFooter()` | cwd 与分支、`↑输入 ↓输出`、`R` 会话累计缓存读取与 `CH` 最近一次回复的缓存命中率（同 Pi 原生 footer，provider 报告过缓存后才显示）、费用（订阅模型标 `sub`），以及其他扩展的全部状态。一行放不下时，状态移到第二行。 |
